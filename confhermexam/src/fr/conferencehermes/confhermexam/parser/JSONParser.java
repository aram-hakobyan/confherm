...
public class JSONParser {
  ...
  public static Profile parseProfileData(JSONObject uJson) {
    ...
    try {
      if (uJson.has(Constants.KEY_DATA) && uJson.get(Constants.KEY_DATA) != null) {
  ...
  public static ArrayList<Result> parseResults(JSONObject rJson) {
    ...
    try {
      if (rJson.has(Constants.KEY_DATA) && rJson.get(Constants.KEY_DATA) != null) {
  ...
  public static ArrayList<NotesResult> parseExerciseResult(JSONObject exRJson) {
    ...
    try {
      if (exRJson.has(Constants.KEY_DATA) && exRJson.get(Constants.KEY_DATA) != null) {
  ...
  public static ArrayList<ExamExercise> parseExamExercise(JSONObject exEJson) {
    ...
    try {
      if (exEJson.has(Constants.KEY_DATA) && exEJson.get(Constants.KEY_DATA) != null) {
  ...
  public static ArrayList<TrainingExercise> parseTrainingExercises(JSONObject json) {
    ArrayList<TrainingExercise> exercises = new ArrayList<TrainingExercise>();

    try {
      if (json.has(Constants.KEY_DATA) && json.get(Constants.KEY_DATA) != null) {
  ...
  public static Exercise parseExercise(JSONObject json) {
    ...
    try {
      if (json.has(Constants.KEY_DATA) && json.get(Constants.KEY_DATA) != null) {
        ...
        JSONObject filesExercise = (JSONObject) data.getJSONObject("exercise_files");
        JSONArray imgArrayExercise = (JSONArray) filesExercise.getJSONArray("image");
        if (imgArrayExercise.length() != 0) {
          JSONObject imgObjExercise = (JSONObject) imgArrayExercise.get(0);
          eFiles.put("image",
              imgObjExercise.getString("file") != null ? imgObjExercise.getString("file") : "");
        } else
          eFiles.put("image", "");
        JSONArray soundArrayExercise = (JSONArray) filesExercise.getJSONArray("sound");
        if (soundArrayExercise.length() != 0) {
          JSONObject soundObjExercise = (JSONObject) soundArrayExercise.get(0);
          eFiles.put("sound",
              soundObjExercise.getString("file") != null ? soundObjExercise.getString("file") : "");
        } else
          eFiles.put("sound", "");
        JSONArray videoArrayExercise = (JSONArray) filesExercise.getJSONArray("video");
        if (videoArrayExercise.length() != 0) {
          JSONObject videoObjExercise = (JSONObject) videoArrayExercise.get(0);
          eFiles.put("video",
              videoObjExercise.getString("file") != null ? videoObjExercise.getString("file") : "");
        } else
          eFiles.put("video", "");
        ...
        for (int k = 0; k < questions.length(); k++) {
          ...
          if (videoArray.length() != 0) {
            ...
            qFiles.put("video", videoObj.getString("file") != null ? videoObj.getString("file")
                : "");
          } else
            qFiles.put("video", "");
          ...
          JSONObject correctionFiles = (JSONObject) qObj.getJSONObject("question_correction_files");
          JSONArray imgArrayCorrection = (JSONArray) correctionFiles.getJSONArray("image");
          if (imgArrayCorrection.length() != 0) {
            JSONObject imgObjCorrection = (JSONObject) imgArrayCorrection.get(0);
            qFilesCorrection.put("image",
                imgObjCorrection.getString("file") != null ? imgObjCorrection.getString("file")
                    : "");
          } else
            qFilesCorrection.put("image", "");
          JSONArray soundArrayCorrection = (JSONArray) files.getJSONArray("sound");
          if (soundArrayCorrection.length() != 0) {
            JSONObject soundObjCorrection = (JSONObject) soundArrayCorrection.get(0);
            qFilesCorrection.put("sound",
                soundObjCorrection.getString("file") != null ? soundObjCorrection.getString("file")
                    : "");
          } else
            qFilesCorrection.put("sound", "");
          JSONArray videoArrayCorrection = (JSONArray) files.getJSONArray("video");
          if (videoArrayCorrection.length() != 0) {
            JSONObject videoObjCorrection = (JSONObject) videoArrayCorrection.get(0);
            qFilesCorrection.put("video",
                videoObjCorrection.getString("file") != null ? videoObjCorrection.getString("file")
                    : "");
          } else
            qFilesCorrection.put("video", "");
  ...
  public static String parseCorrections(JSONObject json) {
    ...
    try {
      if (json.has(Constants.KEY_DATA) && json.get(Constants.KEY_DATA) != null) {
        ...
        JSONArray corrections = data.getJSONArray("question_corrections");
  ...
  public static ArrayList<Correction> parseResultCorrections(JSONObject json, String directory) {
    ArrayList<Correction> correctionsList = new ArrayList<Correction>();
      ...
      if (json.has(Constants.KEY_DATA) && json.get(Constants.KEY_DATA) != null) {
        ...
        for (int i = 0; i < corrections.length(); i++) {
          ...
          if (obj.has("question_correction_files"))
            try {
              HashMap<String, String> qFilesCorrection = new HashMap<String, String>();
              JSONObject correctionFiles =
                  (JSONObject) obj.getJSONObject("question_correction_files");
              JSONArray imgArrayCorrection = (JSONArray) correctionFiles.getJSONArray("image");
              if (imgArrayCorrection.length() != 0) {
                JSONObject imgObjCorrection = (JSONObject) imgArrayCorrection.get(0);
                qFilesCorrection.put(
                    "image",
                    imgObjCorrection.getString("file") != null ? directory
                        + imgObjCorrection.getString("file") : "");
              } else
                qFilesCorrection.put("image", "");
              JSONArray soundArrayCorrection = (JSONArray) correctionFiles.getJSONArray("sound");
              if (soundArrayCorrection.length() != 0) {
                JSONObject soundObjCorrection = (JSONObject) soundArrayCorrection.get(0);
                qFilesCorrection.put(
                    "sound",
                    soundObjCorrection.getString("file") != null ? directory
                        + soundObjCorrection.getString("file") : "");
              } else
                qFilesCorrection.put("sound", "");
              JSONArray videoArrayCorrection = (JSONArray) correctionFiles.getJSONArray("video");
              if (videoArrayCorrection.length() != 0) {
                JSONObject videoObjCorrection = (JSONObject) videoArrayCorrection.get(0);
                qFilesCorrection.put(
                    "video",
                    videoObjCorrection.getString("file") != null ? directory
  ...
  public static ArrayList<TimeSlot> parsePlannig(JSONObject planJson) {
    ...
    try {
      if (planJson.has(Constants.KEY_DATA) && planJson.get(Constants.KEY_DATA) != null) {
  ...
  public static ArrayList<CorrectionsExercise> parseCorrectionsExercises(JSONObject json) {
  ...
  public static ArrayList<CorrectionAnswer> parseCorrectionAnswers(JSONObject json) {
