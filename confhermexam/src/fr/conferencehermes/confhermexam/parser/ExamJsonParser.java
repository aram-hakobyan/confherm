...
public class ExamJsonParser {
  ...
  public static Event parseExamData(JSONObject json, String destDirectory) throws JSONException {
  ...
  public static ArrayList<Exam> parseExams(JSONArray json) throws JSONException {
    ...
    if (json.length() != 0) {
      for (int i = 0; i < json.length(); i++) {
        ...
        for (int j = 0; j < exercisesArr.length(); j++) {
          Exercise ex = parseExercise(exercisesArr.getJSONObject(j), e.getId());
          exercises.add(ex);
        }
  ...
  public static Exercise parseExercise(JSONObject data, int examId) throws JSONException {
    ...
    if (imgArrayExercise.length() != 0) {
      ...
      eFiles.put("image",
          imgObjExercise.getString("file") != null ? directory + imgObjExercise.getString("file")
              : "");
    } else
      eFiles.put("image", "");
    ...
    for (int k = 0; k < questions.length(); k++) {
      ...
      if (imgArray.length() != 0) {
        ...
        qFiles.put("image", imgObj.getString("file") != null ? directory + imgObj.getString("file")
            : "");
      } else
        qFiles.put("image", "");
      ...
      if (soundArray.length() != 0) {
        ...
        qFiles.put("sound",
            soundObj.getString("file") != null ? directory + soundObj.getString("file") : "");
      } else
        qFiles.put("sound", "");
      ...
      if (videoArray.length() != 0) {
        ...
        qFiles.put("video",
            videoObj.getString("file") != null ? directory + videoObj.getString("file") : "");
      } else
        qFiles.put("video", "");
