...
public class ExamineFragment extends Fragment {
  ...
  public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
    View fragment = inflater.inflate(R.layout.activity_examine, container, false);
    progressBarExamin = (ProgressBar) fragment.findViewById(R.id.progressBarExamin);
  ...
  public void onResume() {
      ...
      public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        for (int j = 0; j < dbExams.size(); j++) {
          if (clickedExam.getId() == dbExams.get(j).getId()) {
            ...
            clickedExam.setIsAlreadyPassed(dbExams.get(j).getIsAlreadyPassed());
            break;
          }
        }
        ...
        if (password != null) { // exam is downloaded
            ...
            if (canStartExam(clickedExam)) {
              if (clickedExam.getIsAlreadyPassed() == 0 || examIsConference) {
                ...
                Utilities.showAlertDialog(getActivity(), "Attention",
                    getResources().getString(R.string.exam_already_passed_alert));
              }
            } else {
              Utilities.showAlertDialog(getActivity(), "Attention", "You can't start exam now.");
            }
          ...
          if (clickedExam.getStatus() == 3)
            Utilities.showAlertDialog(getActivity(), "Attention",
                getResources().getString(R.string.exam_not_downloaded_alert));
    ...
    if (Utilities.isNetworkAvailable(getActivity())) {
      ...
      aq.ajax(Constants.EXAM_LIST_URL, params, JSONObject.class, new AjaxCallback<JSONObject>() {
        @Override
  ...
  public void submit() {
    Utilities.showOrHideActivityIndicator(getActivity(), 0, "Synchronisation...");
    ArrayList<ExerciseAnswer> exerciseAnswers = db.getAllExerciseAnswers();
    if (!exerciseAnswers.isEmpty()) {
      try {
        for (int i = 0; i < exerciseAnswers.size(); i++) {
          ...
          if (ea.getIsSent() == 0) {
            ...
            ExamJsonTransmitter transmitter = new ExamJsonTransmitter(getActivity(), ea);
    ...
    builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
      ...
      public void onClick(DialogInterface dialog, int which) {
        if (!input.getText().toString().trim().isEmpty()
            && input.getText().toString().trim().equalsIgnoreCase(password)) {
          Intent intent = new Intent(getActivity(), ExamExercisesActivity.class);
