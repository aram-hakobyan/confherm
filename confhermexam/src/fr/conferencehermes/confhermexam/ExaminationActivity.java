...
public class ExaminationActivity extends Activity implements OnClickListener {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (getIntent().getBooleanExtra("conference", false)) {
      ...
      toggleTimer.setOnCheckedChangeListener(new OnCheckedChangeListener() {
        @Override
        public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
          if (isChecked) {
            ...
            exerciseTimer = new CounterClassForExercise(timeToResumeExercise, 1000);
            timer.start();
            exerciseTimer.start();
            TIMER_PAUSED = false;
          }
    ...
    timer.start();
    exerciseTimer = new CounterClassForExercise(exercise.getDuration() * 1000, 1000);
    exerciseTimer.start();
    ...
    String key =
        "exercise_passed" + String.valueOf(event_id) + String.valueOf(exam_id)
            + String.valueOf(exercise_id);
  ...
  private void selectQuestion(Question q, int position) {
    ...
    if (q.getType().equalsIgnoreCase("1")) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        try {
          if (answersJson != null)
            editText.setText(answersJson.getJSONArray("answers").getString(i));
        } catch (JSONException e) {
          e.printStackTrace();
        } catch (Exception e) {
          e.printStackTrace();
  ...
  public void showAlertDialogWhenExerciseTimeIsOver(Context context, String title, String message)
      ...
      if (currentQuestion.getType().equalsIgnoreCase("2")) {
        ...
        for (EditText editText : editTextsArray) {
          if (editText.getText().length() != 0) {
            ...
            try {
              String answer = new String(text.getBytes("UTF-8"), "ISO-8859-1");
              answers.put(answer);
  ...
  public class CounterClassForExercise extends CountDownTimer {
    ...
    public CounterClassForExercise(long millisInFuture, long countDownInterval) {
      super(millisInFuture, countDownInterval);
    }
    ...
    public void onFinish() {
      ...
      if (!getIntent().getBooleanExtra("conference", false)) {
        try {
          showAlertDialogWhenExerciseTimeIsOver(ExaminationActivity.this, "Attention",
              getResources().getString(R.string.exercise_finished_alert));
